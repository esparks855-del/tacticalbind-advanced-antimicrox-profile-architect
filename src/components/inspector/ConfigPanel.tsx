  ...
  if (!selectedButtonId || !selectedButton) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-zinc-500 bg-zinc-950 border-l border-zinc-800 p-8 text-center">
        ...
        <h3 className="text-xl font-bold text-zinc-300 mb-2">Ready to Configure</h3>
        <p className="text-sm text-zinc-500 max-w-[240px] leading-relaxed">
          Select any button on the controller schematic to begin mapping actions.
        </p>
        <div className="mt-8 text-left w-full bg-zinc-900/50 rounded-lg border border-zinc-800 p-4">
            ...
            <ul className="space-y-2 text-xs text-zinc-400">
                ...
                <li className="flex items-start gap-2">
                    <span className="text-zinc-600">•</span>
                    <span>Use <strong>Hold</strong> to create "Shift Layers" (like holding Shift on a keyboard).</span>
                </li>
                <li className="flex items-start gap-2">
                    <span className="text-zinc-600">•</span>
                    <span><strong>Macros</strong> can perform complex combos with delays.</span>
                </li>
            </ul>
        </div>
      </div>
    ...
    if (slot.actionId) {
        ...
        return (
            <div className="flex items-center gap-2 text-amber-400">
                ...
                <span className="text-xs bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-400 font-mono border border-zinc-700">
                    {action?.defaultKey}
                </span>
            </div>
        );
                    ...
                    <div className="space-y-6">
                        ...
                        <TabsContent value="tap" className="mt-0 space-y-4">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <h3 className="text-sm font-medium text-zinc-300">Primary Action</h3>
                                        <Tooltip>
                                            <TooltipTrigger>
                                                <HelpCircle className="w-3 h-3 text-zinc-500 hover:text-zinc-300" />
                                            </TooltipTrigger>
                                            <TooltipContent>
                                                <p>Triggered when the button is pressed normally.</p>
                                            </TooltipContent>
                                        </Tooltip>
                                    </div>
                                    {mapping?.slots?.[0] && (
                            ...
                            <div className="space-y-2 pt-4 border-t border-zinc-900">
                                <div className="flex items-center gap-2">
                                    <h3 className="text-sm font-medium text-zinc-300">Switch Set (Toggle)</h3>
                                    <Tooltip>
                                        <TooltipTrigger>
                                            <HelpCircle className="w-3 h-3 text-zinc-500 hover:text-zinc-300" />
                                        </TooltipTrigger>
                                        <TooltipContent className="max-w-[200px]">
                                            <p>Pressing this button will permanently switch the controller to the selected Set. Useful for changing modes (e.g. Infantry to Vehicle).</p>
                                        </TooltipContent>
                                    </Tooltip>
                                </div>
                                <Select
                        ...
                        <TabsContent value="hold" className="mt-0 space-y-4">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <h3 className="text-sm font-medium text-zinc-300">Hold Action</h3>
                                        <Tooltip>
                                            <TooltipTrigger>
                                                <HelpCircle className="w-3 h-3 text-zinc-500 hover:text-zinc-300" />
                                            </TooltipTrigger>
                                            <TooltipContent>
                                                <p>Triggered only when the button is held down.</p>
                                            </TooltipContent>
                                        </Tooltip>
                                    </div>
                                    {mapping?.slots?.[1] && (
                            ...
                            <div className="space-y-2 pt-4 border-t border-zinc-900">
                                <div className="flex items-center gap-2">
                                    <h3 className="text-sm font-medium text-zinc-300">Layer Shift (Hold)</h3>
                                    <Tooltip>
                                        <TooltipTrigger>
                                            <HelpCircle className="w-3 h-3 text-zinc-500 hover:text-zinc-300" />
                                        </TooltipTrigger>
                                        <TooltipContent className="max-w-[200px]">
                                            <p>Temporarily activates the selected Set ONLY while this button is held down. Reverts when released.</p>
                                        </TooltipContent>
                                    </Tooltip>
                                </div>
                                <Select
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        <SelectItem value="none">None</SelectItem>
                                        {profile.sets.filter(s => s.id !== activeSetId).map(s => (
                                            <SelectItem key={s.id} value={s.id}>Shift to {s.name}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                            </div>
