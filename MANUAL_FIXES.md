# 🛠️ Manual Fix Instructions (Critical Persistence Recovery)
**ISSUE:** The project configuration (`package.json`, `main.js`, `preload.js`) may revert to a default template (`c-code-react-runner`) after exporting from the platform. This breaks the Electron desktop build.
**SOLUTION:** After exporting this project to your local machine, **IMMEDIATELY** verify the three files below. If they do not match the content here, copy and paste the code blocks to overwrite them.
---
## 1. `package.json`
*Location: Project Root*
*Purpose: Defines the app name, Electron build scripts, and dependencies.*
```json
{
...
## 2. `main.js`
*Location: Project Root*
*Purpose: Electron main process. Handles window creation and native file dialogs.*
```javascript
...
## 3. `preload.js`
*Location: Project Root*
*Purpose: Bridges the secure gap between Electron main process and the React frontend.*
```javascript
